...
class EMetrics(object):
    ...
    def close(self):
        ...
        if self.test_history:
            training_out =[]
            for i in range(len(self.test_history)):
                training_out.append({'steps':(i+1) , 'accuracy':self.test_history[i]['val_acc']})
            open(os.path.join(folder,"val_dict_list.json"),"w").write(json.dumps(training_out))
